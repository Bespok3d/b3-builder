...
- **This golden cannot be re-captured, so it is never regenerated.** It is real output captured from the
  legacy scripts while they still existed; the migration deleted those scripts repo by repo, so the
  capture tool is gone. The rail's claim is "identical to what the legacy scripts produced", and output
  regenerated by today's core cannot make that claim about itself. Never hand-write, fake, or
  re-snapshot a fixture here.
- **When a live source change turns it red, the sanctioned path is a maintainer decision, in this
  order.** It is written down because improvising it is how a rail gets quietly weakened:
  1. **Read the diff first.** The rail prints what moved. A version, an `updated_at` and a payload hash
     moving together is a plugin release. Anything else moving is the core changing its output, which is
     the defect this rail exists to catch: fix the core, do not touch the fixture.
  2. **A plugin release is reconciled by the maintainer, in the open**, in a change that says which
     upstream release caused it and carries the rail green afterwards. It is never a fixture edit
     slipped into an unrelated change.
  3. **Never** delete the rail, blank an assertion, or widen a comparison to get past it. Weakening the
     rail to make a red go away removes the only proof the port is faithful.
- **The app's monorepo-bundle golden is a different kind of fixture and IS refreshable.** It snapshots
  the manifests of sibling repos that still exist, so it is regenerated by command
  (`npm run golden:refresh` in `Bespok3d-desktop`) and never hand-edited. Do not carry the
  no-recapture rule over to it, and do not carry its refresh command back here: this rail has no
  producer that could honestly rewrite it.
