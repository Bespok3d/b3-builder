...
// Finalize the built atoms' placeholder download_url, changelog_url and doc_url values with the real
// GitHub release asset URLs, in place in the output dir. The registry step writes every atom with
// source-relative placeholders (the core stays GitHub-agnostic, ADR-0041); this Action helper swaps in
// the release asset URLs collected while uploading, so each atom is publishable as-is. An atoms-only
// repo (no sub-list) registers by copying these finalized atoms into an index-of-lists' atom pool.
...
function finalizeAtomFile(atomPath: string, assetUrlByFilename: Record<string, string>): boolean {
  ...
  const released = finalizeDownloadUrl(atom, assetUrlByFilename)
  const finalized = finalizeDocUrls(released, assetUrlByFilename)
